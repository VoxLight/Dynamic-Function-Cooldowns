...
def dynamic_cooldown(
    ...
    *,
    ...
    time_period: datetime.time
        The time of day to related to ``limit``. This is the
        time to reset the cooldown at.
    bucket: CooldownBucketProtocol
        The :class:`Bucket` implementation to use
        as a bucket to separate cooldown buckets.
    check: Optional[MaybeCoro]
    ...
    CallableOnCooldown
        This call resulted in a cooldown being put into effect
    """
    _cooldown: DynamicCooldown = DynamicCooldown(limit, time_period, bucket, cooldown_id=cooldown_id)
...
class DynamicCooldown(Cooldown):
    """Represents a cooldown that resets at a specific 
    time each day for any given :type:`Callable`."""
    ...
    def __init__(
        ...
        *,
        ...
        self._cache: Dict[_HashableArguments, DynamicCooldownTimesPer] = {}
    ...
    async def __aenter__(self) -> "DynamicCooldown":
        if not self._clean_task:
            self._clean_task = asyncio.create_task(self._keep_buckets_clear())

        bucket: DynamicCooldownTimesPer = self._get_cooldown_for_bucket(self._last_bucket)
        async with bucket:
            return self

    def _cooldown(self, bucket: _HashableArguments) -> DynamicCooldownTimesPer:
        return self._cache[bucket]
    ...
    def _get_cooldown_for_bucket(
        ...
        try:
            return self._cooldown(bucket)
        except KeyError:
            if raise_on_create:
                raise NonExistent

            _bucket = DynamicCooldownTimesPer(self.limit, self.time_period, self)
            self._cache[bucket] = _bucket
            return _bucket

    def get_cooldown_times_per(
        self, bucket: _HashableArguments
    ) -> Optional[DynamicCooldownTimesPer]:
        """
        Return the relevant DynamicCooldownTimesPer object for
        this bucket, returns None if one does not currently exist.
        ...
        -------
        Optional[DynamicCooldownTimesPer]
            The internal :py:class:`DynamicCooldownTimesPer` object
        """
        try:
            return self._get_cooldown_for_bucket(bucket, raise_on_create=True)
        except NonExistent:
    ...
    def clear(
        ...
        try:
            # Evict item from cache only if it
            # is not tracking anything
            _bucket: DynamicCooldownTimesPer = self._cache[bucket]
            if not _bucket.has_cooldown or force_evict:
                del self._cache[bucket]
        except KeyError:
            pass
    ...
    def remaining_calls(self, *args, **kwargs) -> int:
        ...
        bucket: _HashableArguments = self.get_bucket(*args, **kwargs)
        try:
            cooldown_times_per: DynamicCooldownTimesPer = self._get_cooldown_for_bucket(
                bucket, raise_on_create=True
            )
        except NonExistent:
            return self.limit
